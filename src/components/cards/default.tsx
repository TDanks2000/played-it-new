  ...
  return (
    <Card className="group relative m-0 mt-3 w-[200px] overflow-hidden rounded-t-lg border-none p-0 ring-0">
      <CardContent className="m-0 p-0">
        <Link href={`/info/${_data.id}`}>
          <div className="group relative z-0 overflow-hidden rounded-t-lg focus:outline-hidden">
            <IGDBImage
              alt={name}
            ...
            {/* Gradient Overlay */}
            <div className="from-background/75 absolute inset-0 h-full w-full bg-linear-to-t to-transparent" />
          </div>

          <div className="absolute inset-0 z-10 flex flex-col justify-end p-2">
            <div className="flex flex-col gap-1">
              {/* Title */}
              <h3 className="line-clamp-2 text-sm text-pretty group-hover:underline">
                {name}
              </h3>
              <div className="flex flex-row items-center gap-1 truncate">
                {/* Genre */}
                <p className="text-muted-foreground truncate text-xs">
                  {/* DELETE EVERYTHING INCLUDING THE () AND EVERYTHING INSIDE THEM */}
                  {genres?.length > 0
                    ? genres[0]?.name
                        ?.replace(/\(([^)]+)\)/, "")
                        ?.replaceAll("-", " ")
